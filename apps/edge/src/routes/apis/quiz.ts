...
quiz.post('/questions/next', async (c) => {
  const body = (await c.req.json().catch(() => ({}))) as { mode?: Mode; max?: number }
...
quiz.post('/answers/check', async (c) => {
  const b = (await c.req.json()) as { a: number; b: number; op: '+' | '-' | '×'; value: number }
  const correct = b.op === '+' ? b.a + b.b : b.op === '-' ? b.a - b.b : b.a * b.b
